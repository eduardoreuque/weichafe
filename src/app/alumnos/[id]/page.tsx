...
export default async function StudentEditPage({
  ...
  const dailyClassSales = await prisma.dailyClassSale.findMany({
    where: { studentId: id },
              ...
              {monthlyPayments.length === 0 ? (
                ...
                <ul className="space-y-2 text-sm">
                  {monthlyPayments.slice(0, 5).map((payment) => {
                    ...
                    return (
                      <li key={paymentForClient.id} className="rounded-lg border border-slate-200 bg-white p-2">
                        <p className="font-medium">{paymentForClient.discipline}</p>
                        <p className="text-xs text-slate-600">${paymentForClient.amount.toLocaleString("es-CL")}</p>
                        {paymentForClient.receiptId ? (
                          <Link
                            href={`/comprobantes/${paymentForClient.receiptId}`}
